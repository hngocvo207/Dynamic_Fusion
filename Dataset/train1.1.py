...
# ── NEW: choose which GNN backbone to use inside ETH_GBert ──────────────────
parser.add_argument(
    "--gnn_type",
    type=str,
    default="gcn",
    choices=["gcn", "gat", "sage"],
    help="Graph neural network type for vocabulary graph encoding: "
         "gcn (Graph Convolutional Network), "
         "gat (Graph Attention Network), "
         "sage (GraphSAGE). Default: gcn",
)
# ─────────────────────────────────────────────────────────────────────────────
...
parser.add_argument(
    ...
    type=int,
    default=5,
    help="Stop training if validation weighted F1 does not improve for N epochs. Set <= 0 to disable.",
)
parser.add_argument(
    "--early_stop_min_delta",
    type=float,
    default=1e-4,
    help="Minimum weighted F1 improvement required to reset early stopping counter.",
)
...
cfg_gnn_type = args.gnn_type          # ← propagated from CLI
...
# model filename now encodes the gnn_type so different runs don't overwrite each other
model_file_4save = (
    f"{cfg_model_type}{gcn_embedding_dim}_{cfg_gnn_type}_model_{args.ds}_{cfg_loss_criterion}"
...
print(
    ...
    f"\n  GNN type: {cfg_gnn_type}",          # ← new log line
...
weighted_adj_matrix = load_data(
    "/home/ngochv/Dynamic_Feature/data/preprocessed/Multigraph/weighted_adjacency_matrix.pkl"
)
...
def get_pytorch_dataloader(
    ...
    if shuffle_choice == 0:
        return DataLoader(dataset=ds, batch_size=batch_size, shuffle=False,
                          num_workers=0, collate_fn=ds.pad)
        ...
        return DataLoader(dataset=ds, batch_size=batch_size, sampler=sampler,
                          num_workers=0, collate_fn=ds.pad)
...
train_dataloader = get_pytorch_dataloader(train_examples, tokenizer, batch_size, shuffle_choice=1)
...
if will_train_mode_from_checkpoint and os.path.exists(
    ...
    model = ETH_GBertModel.from_pretrained(
        ...
        gnn_type=cfg_gnn_type,          # ← pass gnn_type
    )
...
for epoch in range(start_epoch, total_train_epochs):
    ...
    wandb.log({
        ...
        "gnn_type": cfg_gnn_type,       # ← log gnn_type for easy filtering in WandB
    })
    ...
    if perform_metrics > (perform_metrics_prev + early_stop_min_delta):
        to_save = {
            "epoch": epoch,
            ...
            "gnn_type": cfg_gnn_type,   # ← persist gnn_type in checkpoint
        }
        ...
        print(
            ...
            f"Precision: {valid_precision:.4f}  [gnn={cfg_gnn_type}]"
        )
    else:
        no_improve_epochs += 1
        print(
            f"No valid F1 improvement for {no_improve_epochs} epoch(s). "
            f"Best so far: {perform_metrics_prev:.4f}"
        )

        if early_stop_patience > 0 and no_improve_epochs >= early_stop_patience:
            print(
                f"Early stopping triggered at epoch {epoch}. "
                f"No improvement greater than {early_stop_min_delta} for "
                f"{early_stop_patience} consecutive epoch(s)."
            )
            break
...
if valid_f1_best_epoch >= 0:
    ...
    print("**No epoch improved over the current best validation weighted F1.")
wandb.finish()